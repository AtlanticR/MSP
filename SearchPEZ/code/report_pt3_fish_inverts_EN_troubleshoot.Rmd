...
# this list.files doesn't seem to be used
...
site_map(PEZ_poly_sf,site_sf,land_sf,10)+
...
get_data(db="isdb", data.dir=file.path(data.dir))
wd <- getwd() # store main project directory
...
marfis_sf <- st_read(temp, layer = marfis_shp, quiet=TRUE)
# remove all files created by save_data() (shapefile and CSV)
fl <- list.files(temp,marfis_shp)
for(i in 1:length(fl)) {
  file.remove(fl[i])
}  
setwd(wd)
marfis_intersect <- st_intersection(marfis_sf,PEZ_poly_sf)
...
obis_plot<-site_map(PEZ_poly_sf,site_sf,land_sf,10)+
