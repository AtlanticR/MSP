...
RVCatch_sf<-SelectRV_fn(RVCatch_sf, minYear, studyArea)
rv_SAR_table<-table_rv_SAR(RVCatch_sf)
rv_table <- table_rv(RVCatch_sf)
```
...
RVCatch_sf <- RVCatch_sf %>% extract(geometry, c('long', 'lat'), '\\((.*), (.*)\\)', convert = TRUE)
rv_start_sites<-data.frame(unique(cbind(longitude=RVCatch_sf$lon, latitude=RVCatch_sf$lat)))
rv_end_sites<-data.frame(unique(cbind(longitude=RVCatch_sf$ELONG, latitude=RVCatch_sf$ELAT)))
...
```{r, include=FALSE, cache=FALSE}
isdb_intersect<-SelectISDB_fn(isdb1, studyArea, minYear)
```
