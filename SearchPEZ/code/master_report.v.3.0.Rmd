...
ISDB is a DFO database that contains at-sea fish catch observations from commercial fishing vessels operating in the North West Atlantic. The data provided by ISDB is confidential and cannot be communicated outside of the DFO, given the direct implications of the Rule of 5.
...
```
**Fig.2:** Map of the defined Potential Exposure Zone (PEZ; blue) and proposed project polygon (yellow) with MARFIS and ISDB observation events shown as black points.
...
```
```{r read for obis , echo=FALSE, include=FALSE, cache=FALSE}
source("plot_obis.R")
```
...
```
```{r read for cws , echo=FALSE, include=FALSE, cache=FALSE}
source("plot_cws.R")
```
...
Filtered by quality grade = research
...
```
```{r read for inat , echo=FALSE, include=FALSE, cache=FALSE}
source("plot_inat.R")
```
