...
```{r Define SARs in the Maritime Region, echo=FALSE, include=FALSE, cache=FALSE}
#list of species listed in SARA schedule 1
MARSAR_com_up <- c("ATLANTIC MUD-PIDDOCK","ATLANTIC SALMON","ATLANTIC WOLFFISH","BELUGA WHALE",
            "BLUE WHALE","BROOK FLOATER","FIN WHALE","HARBOUR SEAL LACS DES LOUPS MARINS SUBSPECIES",
            "LEATHERBACK SEA TURTLE","LOGGERHEAD SEA TURTLE","NORTH ATLANTIC RIGHT WHALE",
            "NORTHERN BOTTLENOSE WHALE","NORTHERN WOLFFISH","SOWERBY'S BEAKED WHALE","SPOTTED WOLFFISH",
            "WHITE SHARK","YELLOW LAMPMUSSEL")
MARSAR_sci_up <- c("BARNEA TRUNCATA","SALMO SALAR","ANARHICHAS LUPUS","DELPHINAPTERUS LEUCAS","BALAENOPTERA MUSCULUS",
                "ALASMIDONTA VARICOSA","BALAENOPTERA PHYSALUS","PHOCA VITULINA  MELLONAE","DERMOCHELYS CORIACEA",
                "CARETTA CARETTA","EUBALAENA GLACIALIS","HYPEROODON AMPULLATUS","ANARHICHAS DENTICULATUS",
                "MESOPLODON BIDENS","ANARHICHAS MINOR","CARCHARODON CARCHARIAS","LAMPSILIS CARIOSA")

MARSAR_sci_low <- c("Barnea truncata","Salmo salar","Anarhichas lupus","Delphinapterus leucas","Balaenoptera musculus",
                "Alasmidonta varicosa","Balaenoptera physalus","Phoca vitulina  mellonae","Dermochelys coriacea",
                "Caretta caretta","Eubalaena glacialis","Hyperoodon ampullatus","Anarhichas denticulatus",
                "Mesoplodon bidens","Anarhichas minor","Carcharodon carcharias","Lampsilis cariosa")
...
#obis<-read.csv("C:/Users/puncherg/Documents/NLP/Data/NaturalResources/Species/OBIS_CBIF_iNaturalist/OBIS_CBIF_iNaturalist.csv")
#obis <- obis[which(year(obis$year)>=minYear),]
```

### **Ecosystem Research Vessel (RV) Survey**

THIS HAS BEEN REPLACED WITH OPEN DATA RECORD FEATURED BELOW
...
obis_shp <- st_read("../../../Data/NaturalResources/Species/OBIS_CBIF_iNaturalist/obis2.shp", quiet=TRUE)
...
```{r, echo=FALSE, include=FALSE, cache=FALSE}
rv_OD_shp <- st_read("../../../Data/mar.wrangling/RVSurvey_OpenData/4VSW_2020.shp", quiet=TRUE)
rv_OD <- read.csv("../../../Data/mar.wrangling/RVSurvey_OpenData/4VSW_2020_summaryb.csv")
...
site_map(PEZ_poly,site,land,2)
points(rv_intersect$SLONG.x,rv_intersect$SLAT.x, col="black",pch=16,cex=1.5)
points(rv_intersect$ELONG.x,rv_intersect$ELAT.x, col="red",pch=16,cex=1.5)
```
...
```{r echo=FALSE, results='asis'}
intersect <- st_intersection(sardist,PEZ_poly_st)
x<-as.numeric(nrow(intersect))
CommonName<-intersect$Common_Nam
intersect$Common_Nam[intersect$Common_Nam == "Sowerby`s Beaked Whale"] <- "Sowerby's Beaked Whale"
CommonName<-intersect$Common_Nam
...
```{r read for sardist, echo=FALSE, include=FALSE, cache=FALSE}
source("plot_sardist.R")
```

```{r Fig X PEZ polygon with all maritime SAR distribution polygons, fig.height=8, fig.width=11}
plot_sardist(sardist, PEZ_poly_st, land_sf)

```

```{r read for zoom of sardist, echo=FALSE, include=FALSE, cache=FALSE}
source("plot_sardist_zoom.R")
```

```{r Fig X close up of PEZ polygon with sardist, fig.height=8, fig.width=11}
...
source("Blue_Whale_habitat_GP.R")
```

```{r echo=FALSE, results='asis'}
#function for overlap
blue_whale_habitat_overlap_GP(Blue_Whale_shp=Blue_4326b, PEZ_poly_st= PEZ_poly_st)
```

```{r echo=FALSE, results='asis'}
#function for sector
blue_whale_habitat_sector_GP(Blue_Whale_shp=Blue_4326b, PEZ_poly_st= PEZ_poly_st)
```

```{r echo=FALSE, results='asis'}
#function for activity
blue_whale_habitat_activity_GP(Blue_Whale_shp=Blue_4326b, PEZ_poly_st= PEZ_poly_st)
```

```{r echo=FALSE, results='asis'}
#function for months
blue_whale_habitat_months_GP(Blue_Whale_shp=Blue_4326, PEZ_poly_st= PEZ_poly_st)
```
...
source("leatherback_GP.R")
```

```{r echo=FALSE, results='asis'}
#function for overlap
leatherback_overlap_GP(leatherback_shp=leatherback_shp, PEZ_poly_st= PEZ_poly_st)
```

```{r echo=FALSE, results='asis'}
#function for sector
leatherback_area_GP(leatherback_shp=leatherback_shp, PEZ_poly_st= PEZ_poly_st)
```
...
```{r echo=FALSE, results='asis'}
#function for overlap
EBSA_overlap_GP(EBSA_shp=EBSA_shp, PEZ_poly_st= PEZ_poly_st)
```

```{r echo=FALSE, results='asis'}
#function for location
EBSA_location_GP(EBSA_shp=EBSA_shp, PEZ_poly_st= PEZ_poly_st)
```

```{r echo=FALSE, results='asis'}
#function for bioregion
EBSA_bioregion_GP(EBSA_shp=EBSA_shp, PEZ_poly_st= PEZ_poly_st)
```
