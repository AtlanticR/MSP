...
Information about the polygon used to search databases (this should include info to be provided by the user). Create a user comment box for DM Apps users.
...
```{r echo=FALSE, results='asis'}
cat("Potential Exposure Zone (PEZ) radius:", PEZversion)
```

```{r echo=FALSE, results='asis'}
cat("Query Time Frame:", minYear, "-", maxYear)
```

```{r echo=FALSE, results='asis'}
cat("PEZ features/comments from user:", UserComments)
```
...
```{r read critical habitat files, echo=FALSE, include=FALSE, cache=FALSE}
...
he Species at Risk (SAR) Program is responsible for carrying out DFO’s mandate under the Species at Risk Act (SARA) to protect, recover and conserve all listed aquatic SAR in Canada.
...
```
**Fig. 6:** Map showing Species At Risk Critical Habitat (red), the defined Potential Exposure Zone (PEZ; blue) and proposed project polygon (yellow).

```{r Fig X close up of PEZ polygon, fig.height=8, fig.width=11}
plot_crithab_zoom(ClippedCritHab, PEZ_poly_st, land_sf)
```
**Fig. 7:** Map showing Species At Risk Critical Habitat (red), the defined Potential Exposure Zone (PEZ; blue) and proposed project polygon (yellow).
...
#}
```
**Fig. 5:** Map showing trawl paths from the Research Vessel (RV) surveys (arrows with start and end points in green and red, respectively) within the defined Potential Exposure Zone (PEZ; blue) and proposed project polygon (yellow).
 ...
 **Table 1:** ISDB records of species observed within the provided polygon which have a listed status with SARA, COSEWIC or Wild Species. Data is summarized by species or species group.
...
```
**Fig.5:** Map showing the defined Potential Exposure Zone (PEZ; blue) and proposed project polygon (yellow) with OBIS records shown as black points.
...
```
**Fig. 11:** Map showing Leatherback Sea Turtle Important Habitat (green) relative to the defined Potential Exposure Zone (PEZ; blue) and proposed project polygon (yellow).
...
### **The Whitehead Lab**
Contact: <http://whitelab.biology.dal.ca/>
Tier of confidence: High
Search year: 
Disclaimers, caveats, sources of uncertainty 
...
Contact:<https://open.canada.ca/data/en/dataset/8fafd919-fcbe-43a3-a911-3d9461273441>
Tier of confidence: High
Search year: 
Disclaimers, caveats, sources of uncertainty
...
```
**Fig. 4:** Map showing observations from the North Atlantic Right Whale Consortium (NARWC) Database of priority cetacean species (SARA and/or COSEWIC listings) within the defined Potential Exposure Zone (PEZ; blue) and proposed project polygon (yellow). Absence of a species in the map should be interpreted as an absence of reporting, not as an absence of the species in the area (e.g. sightings information under-represents presence of cetaceans as this search does not include acoustic detection data).
...
```
**Fig. 3:** Map showing observations from the Whale Sightings Database of priority cetacean species (SARA and/or COSEWIC listings) within the defined Potential Exposure Zone (PEZ; blue) and proposed project polygon (yellow). Absence of a species in the map should be interpreted as an absence of reporting, not as an absence of the species in the area (e.g. sightings information under-represents presence of cetaceans as this search does not include acoustic detection data). 
...
**Table 6:** Results showing if the provided polygon overlaps with areas defined as Priority Areas to Enhance Monitoring of several species of cetaceans (Fin whale, Harbour Porpoise, Humpback Whale and Sei Whale). TRUE = polygon does overlap with priority area; FALSE = polygon does NOT overlap with priority area.
...
```
**Fig. 8:** Map showing various Priority Areas to Enhance Monitoring of Cetaceans.
...
```
**Fig. 9:** Map showing Blue Whale Important Habitat (sky blue), the defined Potential Exposure Zone (PEZ; blue) and proposed project polygon (yellow).

```{r Fig X close up of PEZ polygon and bw habitat, fig.height=8, fig.width=11}
plot_bw_hab_zoom(Blue_4326, PEZ_poly_st, land_sf)
```
**Fig. 10:** Map showing Blue Whale Important Habitat (sky blue) relative to the defined Potential Exposure Zone (PEZ; blue) and proposed project polygon (yellow).
...
### **Search results for other species**
...
<https://obis.org/dataset/80997ac3-87cb-411c-ab1f-334adec45050) via ERDDAP and R package rerdda>
