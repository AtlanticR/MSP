...
```{r echo=FALSE, results='asis', caption="Priority species with observations contained in the CWS_ECCC database within the search polygon area."}
CWS_query_output_table<-if(x < 1){
  ""
} else {
  kable(cws_table4, row.names = F, caption="Species at Risk with observations contained in the CWS database within the search polygon area.") %>%
    kable_styling(bootstrap_options = "striped", full_width = F, position = "left")
}
...
INAT_query_output_table<-if(x < 1){
  ...
  kable(inat_table4, row.names = F, caption="Species at Risk with observations contained in the iNaturalist database within the search polygon area.") %>%
    kable_styling(bootstrap_options = "striped", full_width = F, position = "left")
