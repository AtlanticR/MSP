...
pl <- pl[-grep("xml$",pl)] # remove xml files from list where 'xml' ends the string
# if there is no .lock file this command seems to delete all values from the list
...
PEZ_poly_sf <- st_read(file.path(polyPath,pl[grep(paste0("PEZ_",AquaSiteName,PEZversion),pl)]))
