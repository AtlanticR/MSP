---
title: "SearchPEZ in Blockhouse, St. Andrews, NB"
author: Synthesis prepared by the MSP and PIER teams for an impact assessment
...
Parks Canada is proposing to repair damages to the seawall located at the St. Andrew's Blockhouse in St. Andrews, NB. In preparation for the Impact Assessment and the DFO review we are requesting information on aquatic species, both marine and intertidal, protected species and invasive species in the general area of the Blockhouse (Passamaquoddy Bay). PIER propared Predicted Exposure Zone (PEZ) polygons taht were then used in this document to search regional databases (**2002 - 2018**) and summarize species that have been recorded. This information is used to determine if other, more site-specific, information is available on species presence for PEZ as a complement to the information provided by the request. We recommned to use species lists generated in this synthesis to be circulated to units in Maritimes Science to verify and compliment the information provided (e.g. spatial and temporal presence/absence of species in the area). 
...
Fisheries and Oceans Canada (DFO) has conducted Research Vessel (RV) surveys in the Maritimes Region using a standardized protocol. For more information see <https://waves-vagues.dfo-mpo.gc.ca/Library/4062092x.pdf>. 

There are no records in this location.
...
kable(marfis_freq) %>%
  kable_styling(bootstrap_options = "striped", full_width = F, position = "left")
```
Map below shows the location of samples recorded in the MARFIS and ISDB databases (records are summarized in the tables/sections above). The yellow polygon shows the locations of the aquaculture site. The blue polygon displays the PEZ used for this synthesis. Records were cropped to match PEZ. Rule of 5 was not applied to the map below; consequently, this map is for internal DFO use only (see sources of uncertainty below).
